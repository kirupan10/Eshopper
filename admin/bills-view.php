        ...
        <aside class="main-sidebar col-12 col-md-3 col-lg-2 px-0">
          ...
          <div class="nav-wrapper">
            <ul class="nav flex-column">
			  ...
			  <li class="nav-item">
                <a class="nav-link active" href="manage-bills">
                  <i class="material-icons">table_chart</i>
...
<?php $query=mysqli_query($con,"select distinct orders.bill_no,orders.productId,orders.orderDate,orders.paymentMethod,orders.userId,orders.id,users.name,users.contactno from orders join users on orders.userId=users.id 
where orders.bill_no='$bid' ");
$cnt=1;
while($row=mysqli_fetch_array($query)){
	$prid = $row['productId'];
	$query1=mysqli_query($con,"select * FROM products WHERE id='$prid'");
?>                          
                          <td><?php if($cnt==1){ echo "BDCS176". $bid ;} ?></td>
                          <td><?php if($cnt==1){ echo $row['name'];} ?></td>
						  <td><?php if($cnt==1){ echo $row['contactno'];} ?></td>
